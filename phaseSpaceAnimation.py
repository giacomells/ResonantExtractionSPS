...
import elements  # Importing constants and parameters from elements.py
...
septum = xt.LimitRect(min_x=-1., max_x=elements.septum_aperture_size,
...
zeta = 0.0
dpp = np.random.rand(n_part) * elements.DPP
...
particles = line.build_particles(
    ...
    y_norm=y_norm,
    py_norm=py_norm,
    nemitt_x=elements.N_EX,   # Normalized emittance in x direction
    nemitt_y=elements.N_EY,   # Normalized emittance in y direction
)
...
def update(frame):
    ...
    ax.set_xlim(0.0, 0.1)
    ax.set_ylim(-0.002, 0.0)
    ax.axvline(elements.septum_aperture_size, color="red", ls="--", label='Septum Aperture')
